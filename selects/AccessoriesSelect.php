...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Accessories </title>
        <style>
        ul.Accessories li {
...
<!-- end sidebar -->
<?php
$Accessories = '<ul class="Accessories">';
...
$sql="SELECT * FROM Accessories INNER JOIN clothing ON Accessories.item_id = clothing.clothingID ORDER BY Accessories.item_id";
...
while($row = mysqli_fetch_array($result)) {
  $Accessories .= '<li>';
  $Accessories .= '<img src="#" />';
  $Accessories .= '<h4>' . $row['name'] . '</h4>';
  $Accessories .= '<p>' . $row['type'];
  $Accessories .= " " . $row['description'];
  $Accessories .= " " . $row['brand'];
  $Accessories .= " " . $row['color'] . '</p>';

  $Accessories .= '<p>' . "$" . $row['price'];
  $Accessories .= " " . "stock:" . $row['stock'];
  
  $Accessories .= " " . "rating:" . $row['rating'];
  $Accessories .= " " . $row['condition'] . '</p>';
  
  $Accessories .= '<p>' . $row['release_date_year'];
  $Accessories .= " " . $row['release_date_season'] . '</p>';
  $Accessories .= '</li>';
}
mysqli_close($con);
$Accessories .= '</ul>';
