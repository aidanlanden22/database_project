...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Earrings </title>
        <style>
        ul.Earrings li {
...
$sql="SELECT * FROM Accessories INNER JOIN clothing ON Accessories.item_id = clothing.clothingID WHERE type='earrings' ORDER BY Accessories.item_id";
...
while($row = mysqli_fetch_array($result)) {
  $Earrings .= '<li>';
  $Earrings .= '<img src="#" />';
  $Earrings .= '<h4>' . $row['name'] . '</h4>';
  $Earrings .= '<p>' . $row['description'];
  $Earrings .= " " . $row['brand'];
  $Earrings .= " " . $row['color'] . '</p>';

  $Earrings .= '<p>' . "$" . $row['price'];
  $Earrings .= " " . "stock:" . $row['stock'];
  
  $Earrings .= " " . "rating:" . $row['rating'];
  $Earrings .= " " . $row['condition'] . '</p>';
  
  $Earrings .= '<p>' . $row['release_date_year'];
  $Earrings .= " " . $row['release_date_season'] . '</p>';
  $Earrings .= '</li>';
}
mysqli_close($con);
$Earrings .= '</ul>';
