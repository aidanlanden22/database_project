...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Scarves </title>
        <style>
        ul.Scarves li {
...
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="z-index:3;width:250px" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey" style="font-weight:bold">
    ...
    <div id="demoAcc" class="w3-bar-block w3-hide w3-padding-large w3-medium">
      ...
      <a href="ScarvesSelect.php" class="w3-bar-item w3-button w3-light-grey">Rings</a>
      <a href="EarScarvesSelect.php" class="w3-bar-item w3-button w3-light-grey">Earrings</a>
...
$sql="SELECT * FROM Accessories INNER JOIN clothing ON Accessories.item_id = clothing.clothingID WHERE type='scarf' ORDER BY Accessories.item_id";
...
while($row = mysqli_fetch_array($result)) {
  $Scarves .= '<li>';
  $Scarves .= '<img src="#" />';
  $Scarves .= '<h4>' . $row['name'] . '</h4>';
  $Scarves .= '<p>' . $row['description'];
  $Scarves .= " " . $row['brand'];
  $Scarves .= " " . $row['color'] . '</p>';

  $Scarves .= '<p>' . "$" . $row['price'];
  $Scarves .= " " . "stock:" . $row['stock'];
  
  $Scarves .= " " . "rating:" . $row['rating'];
  $Scarves .= " " . $row['condition'] . '</p>';
  
  $Scarves .= '<p>' . $row['release_date_year'];
  $Scarves .= " " . $row['release_date_season'] . '</p>';
  $Scarves .= '</li>';
}
mysqli_close($con);
$Scarves .= '</ul>';
