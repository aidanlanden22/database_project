...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Necklaces </title>
        <style>
        ul.Necklaces li {
...
$sql="SELECT * FROM Accessories INNER JOIN clothing ON Accessories.item_id = clothing.clothingID WHERE type='necklace' ORDER BY Accessories.item_id";
...
while($row = mysqli_fetch_array($result)) {
  $Necklaces .= '<li>';
  $Necklaces .= '<img src="#" />';
  $Necklaces .= '<h4>' . $row['name'] . '</h4>';
  $Necklaces .= '<p>' . $row['description'];
  $Necklaces .= " " . $row['brand'];
  $Necklaces .= " " . $row['color'] . '</p>';

  $Necklaces .= '<p>' . "$" . $row['price'];
  $Necklaces .= " " . "stock:" . $row['stock'];
  
  $Necklaces .= " " . "rating:" . $row['rating'];
  $Necklaces .= " " . $row['condition'] . '</p>';
  
  $Necklaces .= '<p>' . $row['release_date_year'];
  $Necklaces .= " " . $row['release_date_season'] . '</p>';
  $Necklaces .= '</li>';
}
mysqli_close($con);
$Necklaces .= '</ul>';
