...
$sql="SELECT * FROM Accessories INNER JOIN clothing ON Accessories.item_id = clothing.clothingID WHERE type='hat' ORDER BY Accessories.item_id";
...
while($row = mysqli_fetch_array($result)) {
  ...
  $Hats .= '<h4>' . $row['name'] . '</h4>';
  $Hats .= '<p>' . $row['type'];
  $Hats .= " " . $row['description'];
  $Hats .= " " . $row['brand'];
  $Hats .= " " . $row['color'] . '</p>';

  $Hats .= '<p>' . "$" . $row['price'];
  $Hats .= " " . "stock:" . $row['stock'];
  
  $Hats .= " " . "rating:" . $row['rating'];
  $Hats .= " " . $row['condition'] . '</p>';
  
  $Hats .= '<p>' . $row['release_date_year'];
  $Hats .= " " . $row['release_date_season'] . '</p>';
  $Hats .= '</li>';
}
mysqli_close($con);
$Hats .= '</ul>';
