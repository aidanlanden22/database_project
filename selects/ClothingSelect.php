...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Clothing </title>
        <style>
        ul.Clothing li {
...
while($row = mysqli_fetch_array($result)) {
  ...
  $Clothing .= " " . $row['pageHitsTotal'];
  $Clothing .= " " . $row['pageHitsPerDay'];
  $Clothing .= '<p>' . $row['description'];
  $Clothing .= " " . $row['brand'];
  ...
  $Clothing .= " " . "rating:" . $row['rating'];
  $Clothing .= " " . $row['condition'] . '</p>';
