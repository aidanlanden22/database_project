...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Boutique </title>
        <style>
        ul.Boutique li {
...
$sql="SELECT * FROM Boutique NATURAL JOIN User ORDER BY Boutique.user_id";
...
while($row = mysqli_fetch_array($result)) {
  $Boutique .= '<li>';
  $Boutique .= '<img src="#" />';
  $Boutique .= '<h4>' . $row['name'] . '</h4>';
  $Boutique .= '<p>' . $row['type'] . '</p>';

  $Boutique .= '<p>' . $row['location'];
  $Boutique .= " " . "rating:" . $row['rating'] . '</p>';
